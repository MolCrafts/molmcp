...
class _BigStringProvider:
    ...
    def register(self, mcp: FastMCP) -> None:
        # Bare name — must not be big_emit (plane prefix forbidden).
        @mcp.tool(annotations=ToolAnnotations(read_only_hint=True))
        def emit(size: int) -> str:
...
class TestResponseLimitMiddleware:
    async def test_truncates_large_response(self):
        server = _server(
            provider=_BigStringProvider(),
            response_limit_bytes=1024,
        )
        result = await server.call_tool("emit", {"size": 5000})
    ...
    async def test_passes_small_response(self):
        ...
        result = await server.call_tool("emit", {"size": 100})
        text = result.content[0].text
...
class TestPathSafetyMiddleware:
    async def test_rejects_traversal(self):
        ...
        server = _server(provider=_PathToolProvider())
